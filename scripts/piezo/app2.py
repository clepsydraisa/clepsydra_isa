...
def update_graph(selected_localizacoes):
    ...
    filtered_data = data[data['localizacao'].isin(selected_localizacoes)].copy()

    # Ordenar os dados por data de forma crescente
    filtered_data = filtered_data.sort_values(by='data', ascending=True)
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            autorange=True  # Garantir que o eixo X se ajuste automaticamente
        )
    )
