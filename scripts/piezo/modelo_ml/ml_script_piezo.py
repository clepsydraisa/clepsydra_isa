...
try:
    working_dir = Path(__file__).parent.parent.parent  # Working directory from script location
except:
    working_dir = Path(r"C:\Users\mjmartins\OneDrive - Universidade de Lisboa\Documentos\Clepsydra_ISA\clepsydra_isa-main")  # Fallback para pasta local

class GroundwaterTrendAnalysis:
    """
    Streamlined groundwater trend analysis for evenly spaced time series.
    Esta classe realiza a análise de tendências para dados piezométricos (GWL).
    """
...
# Função para exportar JSON de tendências (não alterada)
...
# --- SEÇÃO DE MACHINE LEARNING: PREPARAÇÃO E TREINAMENTO ---

# Função para preparar dados para ML usando outputs da análise de tendências
def prepare_data_for_ml(analyzer, external_csv_path=None, lag=3):
    """
    Prepara dados para previsão de GWL.
    - Usa o segmento mais recente preenchido.
    - Adiciona lags de GWL, slope de tendência (de HRMK) e direção.
    - Opcional: Merge com CSV externo de features (ex: precipitação, temp, caudal).
    """
    if not analyzer.filled_segments:
        print("Sem segmentos válidos para ML.")
        return None
    ...
    df['trend_direction'] = pd.Categorical([trend.get('hamed_rao_modification', {}).get('trend', 'no trend')])
    
    # Merge features externas se fornecido
    if external_csv_path:
        external_df = pd.read_csv(external_csv_path, parse_dates=['data'], index_col='data')
        df = df.merge(external_df, left_index=True, right_index=True, how='left').fillna(0)
    
    df = df.dropna()
    df.to_csv(working_dir / "resources" / "ml_data_prepared.csv", index=True)  # Exporta dados preparados
    return df

# Função para treinar modelo XGBoost e prever GWL
def train_predict_gwl(df, poço_id):
    ...
    """
    if df is None:
        print(f"Poço {poço_id}: Sem dados para ML.")
        return None, None
    
    X = df.drop('gwl', axis=1)
    y = df['gwl']
    
    X = pd.get_dummies(X, drop_first=True)  # Codificar categóricas
    ...
    print(f"Poço {poço_id} - RMSE: {rmse}")
    
    model.save_model(working_dir / "resources" / f"model_{poço_id.replace('/', '_')}.json")  # Salva modelo
    return model, rmse

# --- FUNÇÃO PRINCIPAL: ANÁLISE INDIVIDUAL POR POÇO (TENDÊNCIAS + ML) ---

def analyze_single_poco(codigo_poco, external_csv_path=None):
    """
    Função principal para análise individual de um poço.
    1. Carrega dados piezométricos do poço.
    2. Roda análise de tendências.
    3. Exporta JSON de tendências.
    4. Prepara dados para ML usando resultados.
    5. Treina e avalia modelo XGBoost para previsão de GWL.
    """
    ...
    if df.empty:
        print(f"Poço {codigo_poco}: Sem dados disponíveis.")
        return
    ...
    print(f"Tendências exportadas para: {output_path}")
    
    # Prepara dados para ML
    ml_df = prepare_data_for_ml(analyzer, external_csv_path)
    
    # Treina e prevê com ML
    train_predict_gwl(ml_df, codigo_poco)
    print(f"Análise completa para poço {codigo_poco} finalizada.")

if __name__ == '__main__':
    # Exemplo: Analise um poço individual (substitua pelo código desejado)
    # Para features externas, forneça o path do CSV (opcional)
    analyze_single_poco('330/183', external_csv_path=Path(working_dir) / "resources" / "clima_snirh.csv")
