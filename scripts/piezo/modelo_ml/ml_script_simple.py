...
LAG = 6  # Número de lags para ML (aumentado para melhor capturar padrões)
TEST_SIZE = 0.2  # Proporção para teste
RANDOM_STATE = 42
MIN_GAP_MONTHS = 6  # Lacuna mínima para segmentação
ALPHA = 0.05  # Significância estatística
FORECAST_MONTHS = 6  # Número de meses para prever (aumentado para 6)
...
class GroundwaterTrendAnalysis:
    ...
    Versão simplificada sem variáveis exógenas.
    """
...
def prepare_data_for_ml(analyzer, lag=LAG):
    """
    Prepara dados para previsão de GWL usando apenas dados do nível de água.
    Versão simplificada sem variáveis exógenas.
    """
    ...
    # Adicionar lags (aumentado para 6 lags)
    ...
    df = df.dropna()
    if not df.empty:
        df.to_csv(BASE_DIR / "resources" / "ml_data_prepared_simple.csv", index=True)
...
def train_predict_gwl(df, poço_id):
    ...
    # Salvar modelo
    model_path = MODELS_DIR / f"model_simple_{poço_id.replace('/', '_')}.json"
...
def predict_future_gwl(model, last_data, trend_results, months_ahead=FORECAST_MONTHS):
    """
    ...
    for month in range(1, months_ahead + 1):
        ...
        new_row = pd.DataFrame([{
            ...
            'gwl_lag_4': current_data.iloc[-4]['gwl'] if len(current_data) > 3 else current_data.iloc[-1]['gwl'],
            'gwl_lag_5': current_data.iloc[-5]['gwl'] if len(current_data) > 4 else current_data.iloc[-1]['gwl'],
            'gwl_lag_6': current_data.iloc[-6]['gwl'] if len(current_data) > 5 else current_data.iloc[-1]['gwl'],
...
def plot_results(analyzer, model_results, predictions, codigo_poco):
    ...
    if model_results and 'y_test' in model_results:
        ...
        ax1.plot(test_dates, model_results['y_test'], 'go', markersize=3, label='Dados de teste', alpha=0.6)
        ax1.plot(test_dates, model_results['y_pred'], 'mo', markersize=2, label='Previsões (teste)', alpha=0.5)
    
    if predictions is not None:
        # Plotar previsões futuras
        ax1.plot(predictions.index, predictions['gwl_previsto'], 'orange', linewidth=2, label='Previsões futuras', alpha=0.7)
        ax1.scatter(predictions.index, predictions['gwl_previsto'], c='orange', s=30, alpha=0.7, zorder=5)
    
    ax1.set_title(f'Groundwater Level - Poço {codigo_poco}\nHistórico vs Previsões (Modelo Simplificado)')
    ax1.set_xlabel('Data')
    ...
    # Salvar gráfico
    plot_path = RESULTS_DIR / f"plot_simple_{codigo_poco.replace('/', '_')}.png"
...
def analyze_groundwater_trends(df, date_col='data', value_col='nivel', 
    ...
    log("Starting 4-step groundwater trend analysis (Simplified)...")
...
def analyze_single_poco_simple(codigo_poco):
    """
    Função principal para análise individual de um poço.
    Versão simplificada sem variáveis exógenas.
    """
    log(f"=== ANÁLISE SIMPLIFICADA PARA POÇO {codigo_poco} ===")
    
    # 1. Carregar dados do poço
    df = load_poco_data(codigo_poco)
    if df is None:
        return None
    ...
    if model is not None and ml_df is not None:
        ...
        if last_segment_trend:
            # Fazer previsão
            predictions = predict_future_gwl(model, ml_df, last_segment_trend)
            
            # Salvar previsões
            forecast_path = RESULTS_DIR / f"forecast_simple_{codigo_poco.replace('/', '_')}.json"
    ...
    # 8. Resumo final com accuracy do modelo
    log(f"{'='*80}")
    log("RESUMO FINAL - MODELO DE PREVISÃO")
    log(f"{'='*80}")
    if model is not None and model_results is not None:
        log(f"Poço: {codigo_poco}")
        log(f"Modelo: XGBoost (Simplificado - apenas dados de GWL)")
        log(f"Horizonte de previsão: {FORECAST_MONTHS} meses")
        log(f"Lags utilizados: {LAG}")
        log(f"")
        log(f"=== MÉTRICAS DE ACCURACY ===")
        log(f"RMSE: {rmse:.4f}")
        log(f"MAE: {np.mean(np.abs(model_results['y_test'] - model_results['y_pred'])):.4f}")
        log(f"R² Score: {model.score(model_results['X_test'], model_results['y_test']):.4f}")
        log(f"MAPE: {np.mean(np.abs((model_results['y_test'] - model_results['y_pred']) / model_results['y_test'])) * 100:.2f}%")
        log(f"")
        log(f"=== PREVISÕES FUTURAS ===")
        if predictions is not None:
            for _, row in predictions.iterrows():
                log(f"  {row.name.strftime('%Y-%m')}: {row['gwl_previsto']:.3f} m")
        log(f"")
        log(f"=== ARQUIVOS GERADOS ===")
        log(f"  - trend_simple_{codigo_poco.replace('/', '_')}.json")
        log(f"  - model_simple_{codigo_poco.replace('/', '_')}.json")
        log(f"  - forecast_simple_{codigo_poco.replace('/', '_')}.json")
        log(f"  - plot_simple_{codigo_poco.replace('/', '_')}.png")
    else:
        log(f"Modelo não pôde ser treinado para o poço {codigo_poco}")
    
    log(f"Análise simplificada para poço {codigo_poco} finalizada.")
    return analyzer, model, rmse, predictions

# ===================== EXECUÇÃO DIRETA =====================
if __name__ == '__main__':
    # Exemplo: Analise um poço individual (versão simplificada)
    analyze_single_poco_simple('330/183')
    
    # Se quiser analisar outro poço, chame a função novamente:
    # analyze_single_poco_simple('331/15') 